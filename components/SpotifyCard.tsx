...
const SpotifyCard = ({ section }: { section: string }) => {
  const { data } = useSWR(
    "/api/nowplaying",
  ...
  return (
    <motion.a
      ...
      className="flex relative rounded-3xl md:col-span-1 col-span-2 overflow-hidden border-[#62DBBE] dark:border-[#1E856D] border-2"
    ...
    >
      <div className="relative -z-50 blur-sm">
        <img
          src={data?.track.album.images[0].url}
          className="mt-1"
          style={{
            display: data?.track.album.images[0].url ? "block" : "none",
          }}
        />
      </div>

      <div className="absolute top-0 -left-5 w-full h-full pointer-events-none">
        <div
          className="flex flex-col text-gray-900 dark:text-gray-100 justify-center xl:ml-7 md:ml-5 mt-5 xl:mt-none ml-4"
          style={{
            position: "absolute",
            top: 0,
            left: 0,
        ...
        >
          <div className="flex flex-col text-gray-400 dark:text-gray-100 justify-center xl:ml-7 md:ml-5 mt-5 xl:mt-none ml-4">
            <div className="items-center justify-center xl:text-2xl lg:text-lg text-xs font-semibold w-52 md:w-40 xl:w-56">
